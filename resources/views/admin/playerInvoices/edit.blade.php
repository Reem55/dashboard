...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.playerInvoice.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.player-invoices.update", [$playerInvoice->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('invoice_type') ? 'has-error' : '' }}">
                ...
                <select id="invoice_type" name="invoice_type" class="form-control" required>
                    <option value="" disabled {{ old('invoice_type', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\PlayerInvoice::INVOICE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('invoice_type', $playerInvoice->invoice_type) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
