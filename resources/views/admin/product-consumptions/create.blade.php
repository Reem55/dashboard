...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.product-consumptions.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
             <div class="form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                ...
                <p class="helper-block">
                    {{ trans('cruds.product-consumptions.fields.code_helper') }}
                </p>
            </div>
           ...
           <div class="form-group {{ $errors->has('reason') ? 'has-error' : '' }}">
            ...
            <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                ...
                <input type="text" id="date" name="date" class="form-control date" value="{{ old('date') }}" required>
                @if($errors->has('date'))
                    <p class="help-block">
                        {{ $errors->first('date') }}
                    </p>
            ...
            <div class="form-group {{ $errors->has('quantity') ? 'has-error' : '' }}">
                ...
                <input type="number" id="quantity" name="quantity" class="form-control" value="{{ old('quantity') }}" step="0.01" required>
